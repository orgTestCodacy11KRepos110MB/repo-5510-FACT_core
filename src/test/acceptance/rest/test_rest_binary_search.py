...
def test_binary_search(test_client, fsorganizer):
    _upload_firmware(test_client)
    _wait_for_binary(Path(fsorganizer.generate_path_from_uid(MATCH_FILE_UID)))
    search_id = _post_binary_search(test_client)
    _get_binary_search_result(test_client, search_id)


def _upload_firmware(test_client):
...
def _post_binary_search(test_client):
...
def _get_binary_search_result(test_client, search_id):

...
def _rest_upload_firmware(test_client, fw):
...
def _rest_search(test_client, fw):
...
def _rest_start_compare(test_client, test_fw_a, test_fw_c):
...
def _rest_get_compare(test_client, test_fw_a, test_fw_c):
...
@pytest.mark.SchedulerTestConfig(dict(items_to_analyze=4 * 2 * 3))
@pytest.mark.usefixtures('intercom_backend_binding')
@pytest.mark.skip(reason='TODO Somehow some processes dont get terminated, so this hangs')
def test_run_from_upload_to_show_analysis(
    test_client, test_fw_a, test_fw_c, analysis_finished_event, comparison_finished_event
):
